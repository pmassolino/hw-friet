...
def friet_sponge_duplex(state, sigma, b):
    ...
    state_parity = bytearray_to_bits(state[3].to_bytes(16, 'little'))
    new_state_rate = [state_rate[j] ^ sigma_padded[j] for j in range(130)]
    new_state_parity = [state_parity[j] ^ sigma_padded[j] for j in range(128)]
    new_state_parity[0] = new_state_parity[0] ^ sigma_padded[128]
    new_state_parity[1] = new_state_parity[1] ^ sigma_padded[129]
    ...
    state[3] = int.from_bytes(bits_to_bytearray(new_state_parity), 'little', signed=False)
    state = friet_p(state)
    return state
