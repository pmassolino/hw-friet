...
#include <stdio.h>
#include <string.h>
#include "Vfriet_stream_fpga_lut4.h"
#include "verilated.h"
...
class Testbench {
    public:
        ...
        Testbench(long long stop_time_value=-1, int skipping_cycles=0) {
            dut = new Vfriet_stream_fpga_lut4;
            time_count = 1;
            dut->clk = 0;
            ...
            dut->trace(tfp, 99);
            tfp->open("tb_friet_stream_fpga_lut4.fst");
#else
            tfp = new VerilatedVcdC;
            dut->trace(tfp, 99);
            tfp->open("tb_friet_stream_fpga_lut4.vcd");
#endif
#else
            stop_time_enable = 0;
            stop_time = 0;
